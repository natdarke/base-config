...
Working with NPM packages is the same as working with your own local files using import or require, except:

1. NPM packages are stored in an NPM repository e.g npmjs.com
2. They are listed as dependencies in package.json
3. They need to be installed using `npm install` which downloads the packages and stores them in node_modules
4. When using import or require, you use the package name, not a file path.

An NPM package is usually stored in 2 locations on the internet:
...
2. An NPM repository
    - Purpose
        - Distribution
    - Contains
        - Source code and/or distribution code (created at build time)
